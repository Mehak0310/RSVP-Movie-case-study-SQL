...
-- Assign value 1 if null in column, else 0. 
SELECT	sum(CASE WHEN id IS NULL THEN 1 ELSE 0 END) AS id_null_count,
		...
		sum(CASE WHEN production_company IS NULL THEN 1 ELSE 0 END) AS prod_company_null_count
FROM movie;
...
SELECT '8' AS median_rating,
	   count(*) AS num_of_movies
FROM movie m JOIN ratings r ON r.movie_id = m.id
WHERE median_rating = 8 
AND date_published BETWEEN '2018-04-01 00:00:00' AND
							'2019-04-01 23:59:00'; 
...
SELECT * FROM top_production_houses WHERE prod_comp_rank <= 3;
...
-- Ordered Indian actors on basis of weighted average rating
WITH indian_actor_rating AS 
(
SELECT name AS actor_name, 
...
SELECT *, 
	   DENSE_RANK() OVER (ORDER BY actress_avg_rating DESC) AS actor_rank
FROM indian_actress_rating;
...
INNER JOIN imdb.genre AS g ON g.movie_id = m.id
INNER JOIN imdb.ratings AS r ON r.movie_id = m.id
GROUP BY genre 
LIMIT 3 
), 
...
SELECT genre,
       ...
       -- Replacing INR and $ with blank to make it suitable for ordering
...
-- For each director, find the next movie published date
WITH director_movie_dates AS 
(
SELECT name_id, 
	   name AS director_name,
       date_published AS first_movie_date,
	   LEAD(date_published) OVER (PARTITION BY name ORDER BY name, date_published) AS next_movie_date
FROM movie m 
JOIN director_mapping dm on dm.movie_id = m.id 
JOIN names n ON n.id = dm.name_id
),

-- For each director, find the inter-movie days
director_intermovie_days AS 
(
SELECT name_id, 
	   director_name, 
       datediff(next_movie_date,first_movie_date) AS diff_in_days 
FROM director_movie_dates
),

-- For each director, find the average of inter-movie days
director_avg_intermovie_days AS 
(
SELECT name_id, 
	   director_name, 
       avg(diff_in_days) AS avg_diff_in_days 
FROM director_intermovie_days
...
SELECT dm.name_id AS director_id,
	   ...
	   round(avg_diff_in_days) AS avg_inter_movie_days,
...
JOIN movie m ON dm.movie_id = m.id
JOIN ratings r ON r.movie_id = m.id
JOIN director_avg_intermovie_days did ON did.name_id = n.id
